...
describe('computeRent', () => {
  ...
  it('doubles base rent when the owner has the full colour set and the toggle is on', () => {
    ...
    const tile = state.board.tiles[9]!;
    if (tile.kind !== 'property') throw new Error('expected property tile');
    expect(computeRent(state, 9, 6)).toBe(tile.rentLadder[0] * 2);
  });
  ...
  it('is 0 for a mortgaged property', () => {
    const state = freshState();
    state.ownership[9] = { ownerId: P2, houses: 0, mortgaged: true };
    expect(computeRent(state, 9, 6)).toBe(0);
  });
  ...
  it('charges 25/50/100/200 for airports based on how many the owner holds', () => {
    ...
    expect(computeRent(state, 6, 6)).toBe(25);
    state.ownership[17] = { ownerId: P2, houses: 0, mortgaged: false };
    expect(computeRent(state, 6, 6)).toBe(50);
  });
  ...
  it('is 0 for a hospital, regardless of ownership', () => {
    const state = freshState();
    expect(computeRent(state, 7, 6)).toBe(0); // Central Hospital, unowned
    state.ownership[7] = { ownerId: P2, houses: 0, mortgaged: false };
    expect(computeRent(state, 7, 6)).toBe(0); // owned — still no landing rent
  });
});
